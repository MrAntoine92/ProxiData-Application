...
# A partir de fichiers excell de référence
...
def test_colonne(df):
    if not isinstance(df, pd.DataFrame):
        return "Le fichier ne contient pas d'agences."
    ...
    expected_columns = ["CA Contrats Collectif","ETP EFFECTIF Exploitation (Présence)","Nbre de véhicules Exploitation",
        ...
        "ETP EFFECTIF Agence (Présence)","TOTAL VEHICULES_2","TOTAL AUTRES COUTS_2",
        '% / (CA+Prod.Centralisées)_1', '% / (CA+Prod.Centralisées)_2',
        '% / (CA+Prod.Centralisées)_3', '% / (CA+Prod.Centralisées)_4', '% / (CA+Prod.Centralisées)_5', '% / (CA+Prod.Centralisées)_6', '% / (CA+Prod.Centralisées)_7',
        '% / (CA+Prod.Centralisées)_8', '% / (CA+Prod.Centralisées)_9', '% / (CA+Prod.Centralisées)_10', '% Absences_1', '% Absences_2', 
        "MARGE BRUTE" ,"TOTAL CA + Prod Centralisées", "ACTIVITE", "Date"] 

    
    # Check if all expected columns are present and if the dataframe is not empty
    if set(expected_columns).issubset(df) & (not df.empty):
        return True
    else:
        missing_columns = set(expected_columns) - set(df)
        return f"Le fichier n'a pas les colonnes: {missing_columns} ou est vide."
...
def import_fiche(fiche_path,
    ...
    # Expected column names
    expected_columns = []
    no_sheet_tab = ['Ecarts', 'Contrôle', 'Chauffage', 'Paramètre', 'MS', 'Graph', 'Annuaire', 'DR Maint'] 
...
def select_agences(df, glob_df):
    ...
    df_clean = df_clean.drop(['name_file'], axis=1)
    ...
    df_clean = df_clean.drop(['ACTIVITE'], axis=1)

    # Ajouter les colonnes 'Agence' et 'Region' de glob_df dans df_clean
    df_clean = df_clean.merge(glob_df[['Filtre Agence', 'AGENCE', 'REGION', 'ACTIVITE']], on='Filtre Agence', how='left')
...
def main():
    ...
    if test_colonne(final_df) == True:
        print("Les colonnes sont correctes.")
    else:
        print("erreur")
